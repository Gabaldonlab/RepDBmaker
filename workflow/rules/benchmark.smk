...
rule get_random_seqs:
    output: "results/comparison/input.fa"
    params:
...
rule diamond_random_seqs:
    input: rules.get_random_seqs.output
    output: "results/comparison/hits/{db}_matches.tsv"
    params: db=lambda wcs: str(config["benchmark"]["dbs"][wcs.db]["dmnd"])
    conda: "../envs/benchmark.yaml"
    log: "results/log/comparison/{db}_dmnd.log"
    benchmark: "results/benchmarks/comparison/{db}_dmnd.txt"
    group: "dmnd_bench"
    threads: 28
    shell: """
...
rule mmseqstax_random_seqs:
    input: rules.get_random_seqs.output
    output: "results/comparison/tax/{db}_report"
    params: 
        db=lambda wcs: str(config["benchmark"]["dbs"][wcs.db]["mmseqs"]),
        lca=config["benchmark"]["lca_mode"]
    conda: "../envs/benchmark.yaml"
    log: "results/log/comparison/{db}_mmseqs.log"
    benchmark: "results/benchmarks/comparison/{db}_mmseqs.txt"
...
rule notebook_comparison:
    input: 
        mmseqs=expand("results/comparison/tax/{db}_report", db=list(config["benchmark"]["dbs"].keys())),
        dmnd=expand("results/comparison/hits/{db}_matches.tsv", db=list(config["benchmark"]["dbs"].keys()))
    output: "results/comparison/report.html"
    # conda: "../envs/R.yaml"
    localrule: True
    script: "../notebooks/comparison.Rmd"
